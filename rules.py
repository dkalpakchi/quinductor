...
class MorphValue(Keyword):
    VALUES = ['Ex', 'Bantu16', 'Ben', 'Pass', 'Dis', 'Hab', 'Cau', 'Ger', 'Qot', 'Abs', 'Neg', 'Cons', 'Bantu18', 'Humb', 'Gen', 'Cmp', 'Mult',
              ...
              'Nhum', 'Bantu6', 'Sub', "Short", "Inen", "Ja", "Lainen", "Llinen", "Vs", "U", "Tar", "Digit"] + lang_spec_vals
    grammar = Enum(*(K(v) for v in VALUES))
...
class SubtreePlaceholder:
    ...
    def __str__(self):
        return "<" + self.name + "." + ".".join([str(t) for t in list(self.tag)]) + (" - " + str(self.neg) if hasattr(self, 'neg') and len(self.neg) > 0 else "") + ">"
...
class Template(List):
    grammar = some([question_word, inter_word, NodePlaceholder, SubtreePlaceholder]), optional(blank), "=>",\
        optional(blank), attr('answer', some([inter_word, NodePlaceholder, SubtreePlaceholder]))
...
if __name__ == '__main__':
    load_templates(['tmpl/en/templates/gen_templates.txt'])